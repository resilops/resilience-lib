...
class WorkloadSpec(BaseModel):
    ...
    replicas: int = Field(
        ..., ge=0, description="Desired number of replicas"
    )
...
class WorkloadStatus(BaseModel):
    ...
    spec_generation: Optional[int] = Field(
        default=None,
        description="Generation of the workload spec currently desired"
    )
...
class NamespaceState(BaseModel):
    ...
    workloads: Dict[str, WorkloadState] = Field(
        ...
        description="Workloads keyed by name"
    )
...
class ClusterState(BaseModel):
    ...
    namespaces: Dict[str, NamespaceState] = Field(
        default_factory=dict,
        description="Namespaces keyed by name"
    )
